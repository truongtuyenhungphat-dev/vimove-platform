...
function openChangePasswordModal() {
  ...
  const dlg = document.createElement('div');
  dlg.id = 'changePwdModal';
  dlg.className = 'modal-overlay';
  dlg.style.cssText = 'z-index:10000;display:flex;align-items:center;justify-content:center;';
  dlg.innerHTML = `
    <div class="modal" style="max-width:400px;width:90%;padding:0">
      ...
      <div class="modal-body" style="padding:20px;display:flex;flex-direction:column;gap:14px">
        <div class="form-group">
          ...
          <input type="password" id="pwdCurrent" class="form-control" placeholder="Nhập mật khẩu hiện tại" />
        </div>
        <div class="form-group">
          <label>Mật khẩu mới</label>
          <input type="password" id="pwdNew" class="form-control" placeholder="Ít nhất 6 ký tự" />
        </div>
        <div class="form-group">
          <label>Xác nhận mật khẩu mới</label>
          <input type="password" id="pwdConfirm" class="form-control" placeholder="Nhập lại mật khẩu mới" />
        </div>
        <div id="pwdError" style="color:#EF4444;font-size:13px;display:none"></div>
      </div>
      <div class="modal-footer">
        <button class="btn-outline" onclick="document.getElementById('changePwdModal').remove()">Hủy</button>
        <button class="btn-primary" onclick="saveNewPassword()">💾 Lưu mật khẩu mới</button>
      </div>
    </div>`;
  dlg.addEventListener('click', e => { if (e.target === dlg) dlg.remove(); });
  document.body.appendChild(dlg);
...
async function saveNewPassword() {
  ...
  // Validate
  if (!currentPwd || !newPwd || !confirmPwd) { showErr('⚠️ Vui lòng điền đầy đủ tất cả các ô.'); return; }
  if (newPwd.length < 6) { showErr('⚠️ Mật khẩu mới phải có ít nhất 6 ký tự.'); return; }
  if (newPwd !== confirmPwd) { showErr('⚠️ Mật khẩu xác nhận không khớp.'); return; }

  // Verify current password against DEMO_USERS
  const email = currentUser?.email;
  const userRecord = DEMO_USERS[email];
  if (!userRecord || userRecord.password !== currentPwd) {
    showErr('❌ Mật khẩu hiện tại không đúng.');
    return;
  }
  ...
  try {
    if (window.fbSaveUser) await window.fbSaveUser({ ...userRecord, email, password: newPwd });
  } catch(e) {
    console.warn('saveNewPassword Firebase error:', e);
  }
